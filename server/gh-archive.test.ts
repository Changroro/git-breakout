...
describe("GH Archive aggregation", () => {
  ...
  it("isolates invalid repository names with explicit line diagnostics", () => {
    expect(aggregateGhArchiveLines([
      event("WatchEvent", 1, "owner/valid"),
      event("PushEvent", 2, "invalid-name"),
      event("DeleteEvent", 3, "also-invalid"),
    ], BUCKET_AT)).toEqual({
      buckets: [{
        bucket_at: BUCKET_AT,
      ...
      rejectedLines: [{
        lineNumber: 2,
        reason: "repo.name must use owner/name format",
      }],
    });
  });
